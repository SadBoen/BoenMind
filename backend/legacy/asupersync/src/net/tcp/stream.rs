...
pub struct TcpStream {
    ...
    /// Windows-only timestamp of the first `WSAENOTCONN` on this stream.
    ///
    /// Some Windows network stacks report `getpeername` success before the
    /// TCP connect has truly completed (observed on Win10 19044 with plain
    /// Aliyun/Telecom routes). In that window every send returns 10057, and a
    /// pure retry-counter budget can be exhausted in microseconds by the
    /// busy-wake fallback (no io/timer driver) long before the connect settles
    /// (~30-150ms). The retry therefore also requires a real-time floor so the
    /// connection is given a chance to complete before the error is surfaced.
    #[cfg(target_os = "windows")]
    first_10057_at: Option<std::time::Instant>,
}
...
async fn wait_for_connect_fallback(socket: &Socket) -> io::Result<()> {
    std::future::poll_fn(|cx| {
        ...
        // BoenMind 补丁（Windows）: `getpeername` 在部分 Windows 网络栈上会
        // 在 TCP connect 真正完成前就返回成功（实测 Win10 19044 直连阿里云
        // 路由），仅凭 peer_addr() 判定"已连接"会提前放行，随后首笔
        // send 命中 WSAENOTCONN (10057)。这里改用内核真实的可写状态
        // （WSAPoll 的 WRITABLE 事件 = connect 完成）作为完成信号，
        // 无阈值、无误判。非 Windows 平台保持原 peer_addr 轮询。
        #[cfg(target_os = "windows")]
        {
            use std::os::windows::io::AsRawSocket;
            use windows_sys::Win32::Networking::WinSock::{
                POLLERR, POLLHUP, POLLWRNORM, WSAPOLLFD, WSAPoll,
            };
            ...
            let n = unsafe { WSAPoll(fds.as_mut_ptr(), 1, 0) };
            if n > 0 && (fds[0].revents & (POLLWRNORM | POLLERR | POLLHUP)) != 0 {
                // 可写或出错 → connect 已定局；SO_ERROR 兜底确认
                if let Some(err) = socket.take_error()? {
                    return Poll::Ready(Err(err));
                }
                return Poll::Ready(Ok(()));
...
impl AsyncWrite for TcpStream {
    ...
    fn poll_write(
        ...
        match (&*inner).write(buf) {
            ...
            //
            // BoenMind 补丁: also enforce a real-time floor. On network stacks
            // where getpeername succeeds before connect truly completes, the
            // busy-wake fallback path (no io/timer driver) can burn the whole
            // retry budget in microseconds — long before the connect settles
            // (~30-150ms). The retry now continues for at least 100ms of wall
            // time so a genuinely settling connection survives.
            #[cfg(target_os = "windows")]
            Err(ref e)
                if e.kind() == io::ErrorKind::NotConnected
                    && this
            ...
            {
                this.connect_settle_retries += 1;
                let _ = this.first_10057_at.get_or_insert_with(std::time::Instant::now);
                if let Err(err) = this.register_interest(cx, Interest::WRITABLE) {
                    return Poll::Ready(Err(err));
                }
                Poll::Pending
