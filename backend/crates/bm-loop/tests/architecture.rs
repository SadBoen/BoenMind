//! 架构依赖测试（LoopX L9 吸收）：bm-loop 不得依赖 bm-server / bm-core。
//!
//! bm-loop 是准内核层（依赖方向：protocol ← kernel ← loop ← server）：
//! 提供商配置/HTTP 由集成方（bm-server）从 bm-core 解析后注入
//! （[`bm_loop::llm::LlmConfig`]），loop 自身不碰应用层。检查两层：
//! 1. Cargo.toml 文本解析（dependencies / dev / build / target 限定全扫）；
//! 2. 源码扫描 `use` / `extern crate` / 全限定路径（隐藏依赖不算分离，L9）。
//!
//! 变更纪律：若确需新增对上层 crate 的依赖，先回架构讨论（依赖方向
//! 反转，不是本文件改一行的事）。

use std::path::{Path, PathBuf};

/// loop 禁止依赖的上层 crate（依赖方向：protocol ← kernel ← loop ← server）。
...
fn does_not_depend_on_upper_crates() {
    ...
    for dep in deps.iter().chain(dev_deps.iter()) {
        assert!(
            ...
            "bm-loop 依赖 `{dep}` 破坏依赖方向（loop 层 ← 应用层）——铁律 3，禁止"
        );
    }
}
...
// ============================================================
// 工具：Cargo.toml 依赖区解析（文本级，与 bm-kernel 的守卫同构）
// ============================================================
