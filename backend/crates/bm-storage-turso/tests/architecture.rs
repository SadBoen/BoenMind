//! 架构依赖测试（LoopX L9 吸收）：bm-storage-turso 不得依赖 bm-server / bm-core。
//!
//! 存储层只能向下依赖（bm-kernel / bm-protocol + 外部基础设施）；对应用层
//! 的依赖是破坏性外向边，机器强制禁止。检查两层：
//! 1. Cargo.toml 文本解析（dependencies / dev / build / target 限定全扫）；
//! 2. 源码扫描 `use` / `extern crate` / 全限定路径（隐藏依赖不算分离，L9）。
//!
//! 变更纪律：若确需新增对上层 crate 的依赖，先回架构讨论。

use std::path::{Path, PathBuf};

/// 存储层禁止依赖的上层 crate（依赖方向：protocol ← kernel ← storage ← server）。
...
fn does_not_depend_on_upper_crates() {
    ...
    for dep in deps.iter().chain(dev_deps.iter()) {
        assert!(
            ...
            "bm-storage-turso 依赖 `{dep}` 破坏依赖方向（存储层 ← 应用层）——铁律 3，禁止"
        );
    }
}
