...
**当前状态(2026-09-07)**:v0.0.11 发版(代码/测试/文档/版本号均对齐);**两处目录树合并共用+任务全灭三链根治**(工作目录选择器与文件树同源单棵树+新建文件夹+文件树多选批量删除/任务双开毒化=绑定前置/审批不可达=/admin/approvals 轮询+YOLO 自批/流式硬顶 180s→900s);此前:v0.0.10 任务执行呈现重构与同命令死循环治理(ZCode 风格脑子图标思考卡/彩色文件扩展名胶囊/吸底常驻 Ticker 状态条/连续 5 次同命令同参防空转熔断);更早:v0.0.9 原生工具协议还原+插件管控批(ADR-0023)+工作目录四区选择器;交付全史见 `milestones/HISTORY.md`;
