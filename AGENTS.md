...
**当前状态(2026-09-07)**:v0.0.12 发版(代码/测试/文档/版本号均对齐);v0.0.12 后**外部审查复核批**(报告逐项核实:target-inspector 121MB 编译产物出库+demo 二进制迁 examples+前端 shim 清理;三误报驳回;存档 `milestones/REVIEW-2026-09-07-v0.0.12-external.md`);**W10 限制配置面+长命令后台转轨**(全仓 ~40 项硬编码限制收敛 config/limits.json+设置页「限制与超时」全量可改+热生效/exec 默认 60s→120s、前台硬顶 600s/超限自动转后台+system.job_output 收取+Ticker 徽标,ADR-0024/0025;五家横评 pi/pi_rust/Hermes/DSH/ZCode 见 docs/);此前 v0.0.11:两处目录树合并共用+任务全灭三链根治(工作目录选择器与文件树同源单棵树+新建文件夹+文件树多选批量删除/任务双开毒化=绑定前置/审批不可达=/admin/approvals 轮询+YOLO 自批/流式硬顶 180s→900s);此前:v0.0.10 任务执行呈现重构与同命令死循环治理(ZCode 风格脑子图标思考卡/彩色文件扩展名胶囊/吸底常驻 Ticker 状态条/连续 5 次同命令同参防空转熔断);更早:v0.0.9 原生工具协议还原+插件管控批(ADR-0023)+工作目录四区选择器;交付全史见 `milestones/HISTORY.md`;
