...
**当前状态(2026-09-07)**:v0.0.13 已发版;发版后复盘报告复核批已落 main(修 skills/roles 配置损坏静默覆写清盘隐患+read_mcp_servers 去重+README/INSTALL 文档校准,误报八项驳回存档 `milestones/REVIEW-2026-09-07-v0.0.13-external.md`);**v0.0.13 批内容**=外部审查复核(target-inspector 121MB 出库+demo 二进制迁 examples+前端 shim 清理,三误报驳回存档 `milestones/REVIEW-2026-09-07-v0.0.12-external.md`)+webadmin.rs 2985 行按域拆 11 子模块+OpenCode Go 网关必带 x-opencode-session 头(400 修复,BOEN_OPENCODE_SESSION_ID 可固定见 PLAYBOOK)+Git 历史瘦身(filter-repo 清 target-inspector/target-wms 共 ~456MB,**历史已重写=既有 clone 须重拉**,仓库 pack 132.6→26.2MB);此前 v0.0.12:W10 限制配置面+长命令后台转轨(全仓 ~40 项硬编码限制收敛 config/limits.json+设置页「限制与超时」全量可改+热生效/exec 默认 60s→120s、前台硬顶 600s/超限自动转后台+system.job_output 收取+Ticker 徽标,ADR-0024/0025);此前 v0.0.11:两处目录树合并共用+任务全灭三链根治(工作目录选择器与文件树同源单棵树+新建文件夹+文件树多选批量删除/任务双开毒化=绑定前置/审批不可达=/admin/approvals 轮询+YOLO 自批/流式硬顶 180s→900s);此前:v0.0.10 任务执行呈现重构与同命令死循环治理(ZCode 风格脑子图标思考卡/彩色文件扩展名胶囊/吸底常驻 Ticker 状态条/连续 5 次同命令同参防空转熔断);更早:v0.0.9 原生工具协议还原+插件管控批(ADR-0023)+工作目录四区选择器;交付全史见 `milestones/HISTORY.md`;
