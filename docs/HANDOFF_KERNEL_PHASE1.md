...
| commit | 内容 |
|---|---|
| `0091b75` | 架构 v0.12：阶段 0 复核融合 §十一（原 review 报告删除）+ Steward 自身生命周期与记忆（压缩/分身扶正/淡化三机制 + 记忆插件化） |
| `4b85bb1` | 阶段 1 前置小修全落地（§三清单）：版本化/append 事务/性能/PortBox/deferred/卸载/CI |
| `e2a58fd` | 重构决策执行：vendor→**legacy** 旧代码文件夹（移出 workspace/P11 撤销/P12 登记）+ LoopX 借鉴 L1-L14 入架构 v0.13 §3.6 |
| `6d0271f` | LoopX 源码级验证校准（12 项：8 属实/4 部分属实）+ 追加 L15-L17 |
...
> 继续 BoenMind 阶段 1。交接见 docs/HANDOFF_KERNEL_PHASE1.md（拍板全录、前置小修全落地 181 测试全绿、legacy 迁移完成、LoopX L1-L17 已吸收、两线任务分解 A1-A7/B1-B6）。**动手顺序建议**：① 先加架构依赖测试（L9，bm-protocol 零依赖 + bm-kernel 不依赖 bm-server/bm-core，成本极低）；② 主线 A 先干 A1（真序事件+ToolResult.output，需先查 pi AgentEvent 工具输出位置）；③ 主线 B 先干 B1-B3（拆法 A 拷 legacy 6 文件+host 线程）。注意 Disposer 纪律（register_service 返回的 Disposer 必须交回 apply 的 Vec）与 turso 绑定形态（Option 混合长度需显式分支）两坑。旧引擎代码一律在 backend/legacy/（引用路径已全量更新）。
