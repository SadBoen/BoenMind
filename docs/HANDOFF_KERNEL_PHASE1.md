...
> **当前状态（2026-08-15 夜自主轮：pi 废除收尾③——legacy 删空达成，§十三终点）**：主线 A + 主线 B **全部落地**；**legacy 删空（本会话 0592cab）**：`backend/legacy/pi_agent_rust`（43M/1180 文件）整体删除（引擎=bm-loop 接替、插件兼容=bm-compat 接替），**legacy 目录已退役**；asupersync（唯一存活 vendored 依赖，bm-compat http 栈）与补丁台账迁入 `backend/vendor/`，[patch.crates-io] 路径更新，台账 pi 区 P1-P12 归档为历史（asupersync A1/A2 保留）；顺手修：**bm-core `vendored_example_path` 失效路径**（2026-08-14 重构 vendor→legacy 后一直指向不存在的 `../../vendor/pi_agent_rust/...`，hello/bookmark 普通构建预装静默失效——改为 `../../plugins/` 仓库自带单文件示例，示例插件随迁 R100）+ 全仓失效引用清理（bm-compat 注释/DEPENDENCIES/README、根 README 目录树与引擎描述、CI 注释、架构文档 §十三 终点记录）+ gitignore 补 artifacts/ 与 compression-regression/out/。**验证**：bm-compat 5 套件 27 测试全绿 + bm-core plugins 4 测试 + check/clippy 通过。
> - 阶段化废除 pi：② **chat.rs pi 分支已删除（dfb52f7，2026-08-15 用户拍板——观察期满，pi 退出生产路径）** ③ **legacy 删空达成（0592cab，2026-08-15 夜自主轮）**——残留生态兼容物（bm-core models.json 同步、bm-compat `legacy_pi_mono_code` 模型资产）不再归属 legacy 语义，随各自宿主组件专项收尾（PackageManager/models.json 专项仍待拍板）
...
> - **下一步**：Steward 轮续接剩余（inject 接桌面壳/系统事件采集——桌面壳代码不在此仓库，等壳侧；宿主侧可先做 Windows 系统事件采集器雏形；steward 前端页已就绪可真实验收）；pi 废除收尾只剩 PackageManager/models.json 生态兼容物专项（bm-core 配置路径，待拍板深度）；**CI 提速 = VMware 自托管 runner（已落地，§〇·五 35）**；工具轮候选 = code-graph MCP 实测。
>
...
> 继续 BoenMind 阶段 2（Steward 轮续接/pi 废除收尾）。交接见 docs/HANDOFF_KERNEL_PHASE1.md。**legacy 删空轮（本会话 0592cab）**：pi_agent_rust 删除、asupersync 迁 backend/vendor/、legacy 目录退役（§十三终点达成，§〇·五 36）；**Steward 续接轮（上一会话）**：静默窗口 watchdog + 低成本模型 + boot 汇报 + 前端状态页（b799dc3，§〇·五 33 配套窗口预算修复 18a15e9）；**双开复跑**：bm 923.8K vs pi 1419.5K（省 35%），记忆题双 5/5（产物 artifacts/dual-compare-v2/）；**sccache 已实证回退**（§〇·五 34，CI 恢复 20min 基线）；**CI 提速已落地** = VMware 自托管 runner（§〇·五 35，质量门 3 Rust job 12m22s）。开工前仍先 git pull。
>
> **下一轮动作（都可直接开工，无需再拍板）**：
> 1. **Steward 轮续接剩余**：inject 通道接桌面壳/系统事件采集（桌面壳代码不在本仓库——等壳侧；宿主侧可先做 Windows 系统事件采集器雏形）；steward 前端页已就绪可真实验收（BM_STEWARD_SESSION + 隔离 BOENMIND_HOME + 浏览器设置页看状态卡片）。
> 2. **pi 废除收尾（只剩专项）**：PackageManager/models.json 生态兼容物专项（bm-core config/compaction/plugins 的 models.json 同步逻辑换自有设施——**深度待拍板**）；bm-compat `legacy_pi_mono_code`（models.generated.ts 构建资产）随 bm-compat 生命周期走。
> 3. 可选顺手件：code-graph MCP 实测（新会话生效）、CI 自托管 runner 增量缓存观察。
>
> **注意坑**（详见 §〇·五 16-34 + 既有全量）：MiniMax 流式须 stream_options.include_usage、缓存字段在 prompt_tokens_details.cached_tokens、pi/bm 两组 input 口径不同（对比换算：bm input=全量、pi input=未命中，发送量都按 input+cache_read 但 bm 勿双重计数）、chars/4 中文低估须真实 usage 校准、pi 句柄断连坏死重启+resume、双开 RUST_LOG 须含 bm_loop=info、413 已修（clip_tool_output 双点）+ **400 已修（窗口预算 18a15e9）**、bm-memory 每会话 open facts.md、桥调用首参 secret 不绑定形参、tool_result 事件 content 用 ContentBlock 数组、内置工具 schema 要注册进 ToolRegistry、SELF_TOOLS 跳过搜索类工具是设计、目录型插件须 extension.json、Disposer 纪律、turso 绑定形态、fork 超头拒绝、standalone 起服务 `--features embed`、bm-compat 测试走 `--test host --test load --test execute --test events --test session`、CompatEngine 专用线程、payload 全文不打日志、**引擎优先级 env>settings>默认 bm（反转点 resolve_loop_engine 一处）**、**subagent 坑：(&BTreeMap).clone() 克隆引用须 (*x).clone()、lines().next_line() 返回 Result<Option> 要 transpose、kill_on_drop 传播取消**、`/api/sessions/{id}/events` 是 SSE 流 curl 会挂、Windows curl 中文 JSON 报 invalid unicode、**Steward 坑（§〇·五 30）：管家提示词须覆盖式声明置尾、验收用全新会话（身份历史污染）、inject 的 wake_after_seconds 会被回合内 set_wake 覆盖、验收加速 BM_STEWARD_PACING_MIN_S=10**、**Steward 续接坑（本会话）：静默窗口监视事件日志 head_seq 而非共享 progress（可能属于 chat 路径 agent）、env 配错回落不阻断、boot 汇报走 dispatch 封装防重**、**subagent 子进程坑（§〇·五 31）：协议 pointer 是 camelCase /assistantMessageEvent/delta、子进程无插件引擎（工具面=内置∩csv）、手工 spawn 验收法看协议行特征区分引擎、create_child_session_handle 已删**。
