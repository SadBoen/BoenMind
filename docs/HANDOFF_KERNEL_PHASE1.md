...
1. **M2 独立壳应用起步**（用户拍板形态）：文件树/编辑器/分支图壳层 + 活任务清单（todo/write 事件协议已现成于 bm-protocol + 任务面板）+ **先修 M1 验收报告 §四 ①max_steps 预算 ②grep/find ignore+timeout ③read offset/limit**（工具面是壳的基座）
