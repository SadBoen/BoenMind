...
> **当前状态（2026-08-14/15 交接收口：自研底座全线 + CI 提速待验证）**：主线 A（A1-A7）+ 主线 B（B1-B6）**全部落地**；**Steward 轮**（管家自我驱动三件套：调度器/inject 通道/next_wake_at 落点，真实验收 4 回合闭环，§14.5）；**pi 废除第②步**（subagent 子进程换 bm-loop 脱离 pi SDK，协议形状对齐父侧零改动，真实验收通过）；**顺手件**（记忆注入真实冒烟：facts.md→bm 引擎注入全链路验证通过 + memorize 误触发修复 8bef21e：裸 remember → remember that）；**CI 提速三件套**（9d43884：concurrency 取消旧轮已生效——文档轮 34s 完成/旧轮自动取消；paths-filter 已生效；**sccache 待下一次代码 push 验证**，预期 quality-test 20min → 5-8min）。**阶段 1 完成态：自研引擎默认 + 管家自主醒来 + 子代理跑在自研引擎上——pi 只剩 chat 主路径回退通道与 legacy 待删。**
...
> - **下一步**：Steward 轮续接（OS 层静默窗口定时器/管家低成本模型/inject 接桌面壳——§14.5 候选）或 pi 废除收尾（chat.rs pi 分支删除待拍板/legacy 删空）；**首个代码 push 顺带验证 sccache 提速**；CI 若仍慢 → VMware 自托管 runner（步骤见 §〇·五 32）。
>
> **轮次历史**（细节见 §〇 commit 索引；查实/坑全量见 §〇·五）：
>
> | 轮次 | 内容 | commit | 关键坑/查实 |
> |---|---|---|---|
> | CI 提速轮 | 质量门三件套：concurrency 取消旧轮 + paths-filter 路径过滤 + sccache 对象级缓存 | 9d43884 | 根因实证：rust-cache 命中仅 879MB（依赖树 10GB+）→ test 20min；文档轮已演示 34s 完成 + 旧轮自动取消；**sccache 提速待代码 push 验证** |
