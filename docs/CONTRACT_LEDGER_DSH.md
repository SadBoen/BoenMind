...
>
> 实施进度（2026-08-18 **M3 起步**）：**真 provider 三通道落地**——`kernel-llm::openai`（OpenAI 兼容流式适配器，SSE 逐行解析、tool_calls 增量、usage、torn 纪律）+ `multi`（按 provider 路由的聚合端口）+ `kernel-loop` per-session 模型覆盖（session.selectModel 真实生效）+ `web-server::provider_config`（兼容旧 boenmind `config.toml` 子集：minimax/deepseek/custom，key 缺省读 env `{ID}_API_KEY`）+ `--config` CLI。`llm.providers/models` 真 provider 目录、`llm.discoverModels` 真实探测（`GET {base}/models`，minimax/deepseek 实测同构）。**端到端实测**：真实 MiniMax-M3 流式回复（`docs/conformance/m3-real-provider-verify.mjs` 11/11 PASS）；门禁 2.5 回归 15/15、conformance 17/17 不变。**credentials.{set,unset} 已补**（ref 正则校验、值永不出域、内存态）。
>
> 实施进度（2026-08-18 **M3 轮 2**）：**session.search 已实现**（query trim 1-500 禁 NUL、扫 user/assistant 文本、snippet ≤240 code points 窗口、结果 ≤20、`hasMore:false`）+ **session.fork 已实现**（atSeq 锚定首个 ≥atSeq 的 turn/end、省略/越界回退最后完成 turn、无完成 turn → `fork-unavailable`、复制日志到新会话、持久化+会话表登记+host/session-added 广播）+ **host 流基线帧**（连接时 workspace-changed 全量快照 + 每会话 session-added/session-status）+ **host 实时广播**（workspace create/rename/delete/insertBefore/insertSessionBefore/archiveSession → workspace-changed/workspace-removed/workspace-order-changed/archived-sessions-changed；session create → session-added；prompt → session-status 翻转）。验证：`docs/conformance/m3-r2-verify.mjs` 15/15 PASS + gate25 15/15 + conformance 17/17。未实现方法（返回 bad-request）：goal.*、subagent.*、session.{attachment,updateQueue}、settings.openDocument、host.openPath。RPC 面 28/52。
>
> 实施进度（2026-08-18 **M3 收尾**）：**RPC 面 52/52 全实现**。①respond pending 表（approval 先/question 后、整批校验、`approval/resolved`/`question/resolved` 广播）+ mux 重开重放仍 pending 帧（rpcId 原样复用）；②session/projection 槽（key→(seq,value) 单调、higher-seq-wins、`session/projection` 帧 + history tail projections）+ goal.\* 6 法（wire 契约在 web-server、内存态最小桥、CAS ref、'goal' 投影、clear 留墓碑 null）；③session.attachment（日志引用表 → attachment-error）、session.updateQueue（queue-item-not-found）、settings.openDocument（opened:true）、host.openPath（OS 打开）；④subagent.\* 4 法（wire 契约逐字、无 team 插件时 parentAvailable:false/逐字错误码，不装死）；⑤agentPreset 剩余 5 法（select/read/copy/openDocument/remove 无 authoring 预设 → 逐字错误码）；⑥session.export（ZIP：session.jsonl，头 content-type: application/zip + content-disposition，400/404 分支）；⑦**LLM 层挂载点对齐**：StreamChunk 词汇对齐 DSH `types.ts`（block-start/text-delta/reasoning-delta/tool-call-delta/block-end/usage/finish，index 关联、block-end 携带组装块、usage 先于 finish）+ openai.rs SSE 翻译对齐 `translate.ts`（[DONE] 收尾、usage 缓存剔除 disjoint、EMPTY_RESPONSE、finish 后无块）+ `LlmPort::resolve_model`（contextWindow/maxTokens/reasoning 元数据）+ assistant/chunk 原始块入日志（replay 保真）+ usage 随 assistant/message + turn/end reason 词汇 + ToolCall.arguments 改模型原始 JSON 文本。**验证**：`docs/conformance/m3-r3-verify.mjs` 40/40 PASS + gate25 全过（全新 db）+ conformance 17/17 + m3-r2 15/15 + hot-upgrade-transition-verify PASS + 全部 cargo test 62 过 + clippy 零警告。**遗留 → M3.5**：goal 自动续跑（goal 插件）、team 插件真实子代理执行、settingsNs 热替换 registerAdapter 写面、MAX_STEPS 数值可配置（已加 `LoopRuntime::max_steps` 字段，装配默认 DEFAULT_MAX_STEPS=32）。
>
> 实施进度（2026-08-18 **M3.5 核心收尾 + 官方测试集启动**）：**settingsNs 热替换落地**（registerAdapter 写面，LLM 挂载点差距最后一项）——provider 的 `llm.<id>` settings ns 动态 `baseURL`（`settings_write`/`settings_describe` 支持动态 ns，写后同步适配器、null 恢复装配值）+ `credentials.{set,unset}` 对 `{ID}_API_KEY` ref 同步 key 覆盖（对齐 DSH dynamic-config.spec.ts：key 经 credentials 每请求解析、写后下一请求生效）。**keyless 装配合法化**（provider_config 缺 key 不再跳过；请求 MISSING_CREDENTIAL 可热补）。**`--max-steps` CLI**（`Runtime.max_steps` 字段 + `headless_with_max_steps`）。**7 处官方 wire 差距修复**：HTTP 错误码映射（401/403→AUTH、429→RATE_LIMIT、400→INVALID_REQUEST、500/503→SERVER、其余→HTTP_\<status\>）、未注册 provider→`NO_ADAPTER`、assistant tool-call 轮 content 空串（非 null，API 400 拒 null）、**流错误统一 finish 呈现**（修真 bug：Err chunk 被 loop torn 分支覆盖成 LLM_STREAM 且双回合收尾——TRANSPORT/MALFORMED_RESPONSE/STREAM_CLOSED/MISSING_CREDENTIAL 全改 finish）、keyless→MISSING_CREDENTIAL 引导语。**官方测试集 650 spec 已盘点核心面**（llm 34/session 38/host 33/credentials 6/settings 8/workspace 2/interaction 9 等），差距按 P0/P1/P2 登记于 `docs/DSH_TESTSUITE_ALIGNMENT_2026-08-18.md`。**验证**：cargo test --workspace 67 全过 + clippy 零警告 + conformance 17/17 + gate25 PASS + m3-r3 40/40 + `scripts/hot-replace-verify.mjs` 15/15（新）。**下轮指针**：P0 错误归一化/上下文超限分类/abort 语义 + P1 wire 形状补全 + 官方 spec 镜像 Rust 单测（`docs/HANDOFF_M4_2026-08-18.md`）。
>
> 实施进度（2026-08-18 **M4 P0+P1 全落地**）：**P0-1 错误归一化**——`LlmError` 加结构化字段（code/status/provider_retry_after_ms/request_id）+ `FailureInfo` 终态形状 + `FinishReason::Error.extra` 透传（wire 携带 status/providerRetryAfterMs/requestId 时逐字上 wire、无则只 message/code，镜像 adapter-failure.spec.ts 兜底：message 空→"LLM adapter failed"、code 缺失→UNKNOWN）。**P0-2 内容分类**——`map_http_code(status, body)` 镜像 DSH `httpErrorCode`：401/403→AUTH 优先、quota 判词（任意 status，429+insufficient_quota 等→`QUOTA`）、400+上下文超限判词→`CONTEXT_WINDOW_EXCEEDED`、`status>=500`→SERVER（502 也归）、413 不被内容覆盖；`parse_retry_after`（秒数/HTTP-date，0/非法/过去省略）+ `x-request-id`/`x-deepseek-request-id` 结构化事实入 failure。**P0-3 abort 语义**——`kernel-contracts::AbortSignal`（AtomicBool + tokio watch，wait_aborted 无竞态）、`GenerateOptions.signal`、openai.rs 预 abort 不碰传输 + select_biased 竞争流读/abort（挂起读可被打断）+ send() 阶段穿透 + abort 后连接错误按取消呈现、`ReactLoopAgent.abort()` + `session.cancel` 接线、loop 消费 Cancelled→`TurnEndReason::Aborted`（独立 reason，不再归 Error{ABORTED}）→ wire `{kind:'aborted', code:'ABORTED'}`。**P1-1/2/3 wire 形状**——`stream_options:{include_usage:true}` 恒随请求、`resolveThinking` 镜像（effort 档位校验/off→disabled/会话标题+压缩强制 off/deployment 锁 disabled 拒绝显式 effort）→ `thinking:{type}` + `reasoning_effort` 上 wire、`translate_messages` 重写（text 合并、tool-result 独立 role:tool 消息、空输出哨兵 `'(no output)'`、混合 user 拆多条、reasoning passback 只在 tool-call 轮、image 拒收点预留 UNSUPPORTED_CONTENT）。**P1-4 settings-conflict revision**——`settings_revisions` 单调计数、写带 `expectedRevision` 不匹配 → `settings-conflict{ns,expected,actual}`。**P1-5 session.search astral**——修真 bug：`make_snippet` 字节偏移/字符数混用（query 前多字节字符导致窗口错位），改 char 换算 + astral 测试。**归因头（P0 台账最后一项）**——openai.rs 恒发 `user-agent: boenmind/0.1.0 (+url)` + `x-deepseek-harness-user-id`（`~/.boenmind/.anonymous-user-id` 持久 UUID v4，`wx` 独占创建防并发、best-effort）+ `x-deepseek-harness-session-id`（按需）+ `x-deepseek-harness-compact`（compaction 用途）；**requestId 投影**——`TurnEndReason::Error.request_id` 从 finish failure 结构化事实投影 → events wire `error.requestId`（无则省略）。**官方 spec 镜像单测 24 条新增**（service.spec 判词表/HTTP 状态行保留/Retry-After/归一化兜底/wire 形状/预 abort/流中 abort（mock TCP 端点）/wait_aborted 幂等/SSE 序列（空首增量+推理文本块+默认 stop）/STREAM_CLOSED 空流/DONE 后停止/MALFORMED_RESPONSE/EMPTY_RESPONSE after usage/stream_options/thinking 决议/哨兵/拆条/passback/snippet astral/归因头捕获）。**验证**：cargo test --workspace 91 全过 + clippy 零警告 + conformance 17/17 + gate25 PASS + m3-r3 PASS + hot-replace ALL PASS。**遗留 → P2/插件**：BlockAssembler replayState 剪枝、message 冻结、retry-policy、settings 文件层、session-query 全套（见 `docs/DSH_TESTSUITE_ALIGNMENT_2026-08-18.md` §2 P2）。
