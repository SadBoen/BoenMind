...
- [x] 批5|PluginsPage.tsx 拆分(edcb08f;tsc/lint/build/smoke 绿+插件中心真机截图 08)
- [x] 批6|context.tsx 拆分(d7111d8;utils/recipe 纯函数模块;上下文页真机验收截图 09)
- [x] 批7|F-12 依赖倒置(36bcf64;bm_core::ports::persist 收口+bm-persist 反转 re-export;dev 依赖环→内存桩)
- [x] 批8|F-07 MCP 装配下沉(1ff3da5 characterization + 17f9a2f supervisor,启动/热装载同调)
- [x] 批9(首函数)|spawn_turn 提取 audit_model_invoke(0e44582;793→600 行)
- [ ] 批9(续)|其余 6 长函数同款提取:capability_call_inner/handle_turn_event/dispatch_capability/handle_task_create/spawn_subtask/worker_call;每函数 1 commit;手法参照 spawn.rs 的 audit_model_invoke(段落整体搬移+块尾表达式直返)
- [ ] 收官|Skill S5 残项(ADR-0016 状态更新/性能基线)+ HISTORY/BACKLOG 结转 + 最终推送

## 任务清单(按序执行)

- [x] R1 持久读错误折叠为空收口:handle.rs 启动恢复 8 处 + events_for_session 等——区分「文件不存在(合法空)」与「读/解析失败(拒开/上抛)」,消灭故障消音成假数据
- [x] R2 emit 坏形状事件 seq 空洞:runtime.rs 校验前置,坏事件不占 seq(保 INV-3 连续)
- [x] R3 MCP 治理四件:①reload 强杀旧子进程 ②respawn 去抖+上限(启用 restart_limit)③HttpMcpTransport 请求超时 ④死配置处置
- [x] F1 工具白名单按需挂载:Role 配置可选 allowed_tools(缺省=全量,向后兼容),turn 按白名单过滤 chat_tools
- [x] F2 意图门控软防线:挂载工具时系统提示追加「工具纪律」段(寒暄/纯问答禁动文件与系统工具)
- [x] F3 skill.v0_1 Rust 强类型投影:SkillDefinition 结构体+镜像测试
- [x] F4 context-inspector 插件工具名瘦身:去 context_ 前缀,重建 exe+数据目录换装+重载验证
- [x] F5 web_multisearch Parallel Search 接入(数组参数特例解析,用户 Key 已在库)
- [x] Q1 ESLint 接入前端(先最小规则集,修 autofix;若存量问题>50 只登记不硬修)
- [x] V1 全量回归:validate.py + fmt + clippy --all-targets + workspace test + tsc/build + smoke
- [x] V2 真模型浏览器验收(工具白名单生效/意图门控/MCP 重载) + 截图
- [x] V3 收宫:台账置完成 + HISTORY/BACKLOG 结转 + push
