...
给下一轮对话/协作的接手者。这是「全面回头看 + 连续三轮架构修复」之后的仓库状态快照。
...
**核心插件（loop/llm/tools）只依赖 kernel-contracts + kernel-session + bm-ports**——不再编译期碰任何兄弟插件/功能插件。这是三轮修复的核心成果。
...
### P0（有明确收益）
- **api.rs session 域拆分**：当前 1636 行，session 域（session_list/create/history/search/fork/prompt/models/selectModel 等 ~640 行 + make_snippet/model helper）仍未拆。可拆 `api/session.rs`（模式已定型：pub(super) fn + mod/use + 共享 helper 留主文件）。注意 session 与 llm 共享 model_groups/mock_model_group/current_model——helper 留 api.rs 主文件即可。
- **workspace lints 收口**：根 Cargo.toml 无 `[workspace.lints]`。可加（`cargo clippy` 现在只是默认 warn，没有 deny 级别约束）。

### P1（内聚改进，非紧迫）
- **api.rs 测试区与 dispatch 分离**：当前测试区（~430 行）仍在 api.rs 尾部。若 session 拆走后测试跟着拆到各子模块，api.rs 更瘦。
- **cargo-deny 引入**（license/bans/重复 crate 审计）——Grok 建议过，但需评估 M 大小。
- **`kernel-supervisor` 未来接 M3**：当前是死代码（仅 crate_boundaries 登记），若长期不接可考虑从 workspace members 摘除（注意守卫层表同步）。

### P2（产品功能/体验，非架构）
- **`[compaction]` 前端可视化**：配置段已接线（watermark/keep_recent_ratio/keep_recent_floor/min_middle_tokens），但设置中心「上下文」页还没有对应表单。
- **headless L0 的 `--compact` 支持**：目前只有 web-server 有 --compact 装配，headless 没有（如有需要补）。
- **frontend 的 Config→TDI sharedConfig 漏传**：前端启动只拿到 settings、没拿到 providers（settings.ts 与 TDI 的 sharedConfig 对齐是实现欠账）。
...
- 记忆库见 `C:\Users\Boen\.zcode\cli\memories\projects\boenmind-...\memory/`（project-architecture-fix.md 汇总三轮架构决策）。
