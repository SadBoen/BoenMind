...
- **版本**：产品名保持 BoenMind；**从 v0.1.0 起**（用户拍板：dsh 内核时代的第一个正式版，与 pi 内核时代的 v0.1.x 历史版本无关——这是另一个项目）。
...
- **门禁 4**：v0.1.0 全量回归 + 便携包真实启动（沿用"先本地实测再发版"铁律）。
...
## 八、拍板点（2026-08-17 已全部拍板）

| # | 议题 | 决议 |
|---|---|---|
| 1 | 仓库形态 | **新仓库 `boenmind-dsh`**（独立 git 项目，现有仓库只读参考资产） |
| 2 | 版本号 | **从 v0.1.0 起**（dsh 内核时代第一个正式版） |
| 3 | Node 分发 | **内置 Node runtime（便携包）**——开发期用系统 Node，发布期内置（拍板点 4 浏览器先行不阻塞此设计） |
| 4 | 桌面壳 | **浏览器先行**（web 版先发，Tauri 2 桌面后置） |
| 5 | 插件信任 | **两档**：官方/自研插件宿主进程 + 第三方插件隔离 worker（映射现有普通/资深分级） |
| 6 | 浏览器自动化 | **后置**：M3 团队/管家之后排期（v0.2.x 路线图） |
| 7 | 历史数据 | **只读归档**（现有 turso 不动，不写导入脚本；将来按需再加一次性工具） |
