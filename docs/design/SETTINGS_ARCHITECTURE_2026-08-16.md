...
> 状态：**阶段 1–5 已落地（2026-08-16），阶段 6 可选未做。** 从属于架构宪法 v0.26；与宪法冲突时以本文拍板表（§二）为准。MCP 组合根始终持有 `McpClientManager`（零连接合法，设置页可添加第一个 server；2026-08-17 C P1 已修）。
...
**模板与实例（2026-08-16 用户定调）**：专家预设（agents/*.md）只是**模板**，不是实例。
同一模板可以派出任意多个 Agent 实例——例如「coder」模板派出"码农一号、码农二号"并行干活；
实例持有各自的任务上下文/会话，模板只提供角色/模型/工具面/记忆桶的"出厂配置"。
管理面只维护模板（增删改）；实例的产生与命名随派工机制（subagent 派工、会话绑定），
不在专家预设页另造实例管理界面。
