...
**下一轮候选**：OS 层静默窗口定时器（宿主侧"1 分钟无汇报主动上报"）、管家回合可换低成本模型（成本杠杆）、inject 通道接入桌面壳/系统事件、steward 状态前端展示。
