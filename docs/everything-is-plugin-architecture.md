...
> 状态：**v0.22（迭代中）**——v0.19 = 管家自我驱动三件套落地（§14.5，真实验收通过）：调度器/OS 汇报通道/next_wake_at 落点，set_wake 工具自调节奏，覆盖式管家提示词；v0.20 = Steward 续接轮（§14.5 续接，2026-08-15 夜自主轮）：静默窗口 watchdog + 管家低成本模型（env 可配）+ boot 汇报 + 前端管家状态页 + 工具结果窗口预算裁剪；v0.21 = 架构回头看 + 全网对标（§15，两报告 docs/REVIEW_ARCHITECTURE_2026-08-15.md / docs/REVIEW_LANDSCAPE_2026-08-15.md）；v0.22 = **对话宿主化与场景作用域拍板（§四·B 补充，用户拍板按推荐执行）**：ChatPane 宿主共享组件/会话 app 场景/工具面按场景组装/scopes+槽位 + **内核主权评估（§15.4，用户开题"换 dsh 内核"三轮讨论定调）**：不换不跟随、吸收机制、兼容层吃生态；核心实现方案见 docs/kernel-implementation-plan.md
> 日期：2026-08-14
