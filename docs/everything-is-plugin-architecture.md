...
> 状态：**v0.26（2026-08-17 架构文件校准）**——v0.22 = 对话宿主化与场景作用域（§四·B 补充，已实施）；v0.23 = 应用布局系统（§四·B 补充 2，已实施：dockview 8.1 + DockLayout 宿主，2026-08-15）；v0.24 = 文档整理轮；v0.25 = 皮肤系统与背景特效（§四·B 补充 3）；v0.26 = 文档权威收口（面数/桌面退役/记忆当前态/安全基线回写；不改骨架）。**当前实施状态速览**：内核建成；协议服务面 **14**（含 `provider`）+ 运行期 `mcp`（契约在 bm-mcp；**消费状态以 `docs/EXTENSION_POINTS_REGISTRY.md` 为准，禁止再手写 13/14/10**）；Steward 三件套落地（§14.5）；桌面壳代码已退役、开关占位（§四·B）；编程应用 M1 ✅、M2 活 todo ✅；skill 系统提示按 `session.app` + `skill_scopes` 注入已落地（§四·B 补充）；配置单锁（2026-08-17）。
...
- **skill 场景注入（已落地，2026-08-16 设置中心阶段 3）**：`enabled_skills_prompt(config, app)` 只把「已启用 + `skill_scopes` 命中当前 `session.app`」的技能清单写入系统提示（公共 = 空/`*`）。插件/MCP **工具面**同样按 app 过滤。设置页 ScopePicker / APP 设置页改作用域后 `invalidate`，下一回合按新名单重建。清单仍是名字+简介+路径，全文按需 `read` SKILL.md——这是省上下文，不是场景过滤缺口。
...
### 四·B 补充 3：皮肤系统与背景特效（v0.25，2026-08-16）

**皮肤系统（2026-08-16 落地）**：skins/ 文件夹一皮肤一目录 + html `data-skin` 切换 + 参数化（色调/透明度/模糊）+ 本地背景图/预设壁纸；Aqua 观感纯自研 CSS，HypoMux 观感 Fluent UI 拼装。**风格≠布局、材质不进组件库**——皮肤只改视觉材质，不碰应用结构。
...
M2：任务清单动态演化（10 个任务的清单，中途插入/删除/重排，全程日志可查）——✅ 活 todo + 事件投影已验收（2026-08-15）；分支图 DAG / 项目切换 / 终端已落地。对话宿主化③ skill 系统提示按场景注入亦已落地（见 §四·B 补充），不计入本条原验收句。
