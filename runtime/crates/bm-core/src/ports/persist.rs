...
/// 配置类文件的原子覆盖写(#35 归位:落盘小工具所有权随 F-12 收归端口层,
/// bm-persist 仅 re-export 保持旧路径,Surface 层不得直依赖持久层实现)。
/// `std::fs::write` 原地截断重写,断电/磁盘满/进程崩溃会留下半截文件,
/// 配置损坏 = 下次启动即挂。此处统一「临时文件 + flush + fsync + rename」:
/// 任意时刻盘上只有完整旧文件或完整新文件(rename 单原子)。
/// 目标父目录不存在则创建;Windows 上 rename 经 MoveFileExW(REPLACE_EXISTING)
/// 可覆盖既有文件;临时名带进程内序号(P1-15:防并发写互踩同一 tmp)。
...
mod fs_util_tests {
    ...
    fn filter_lines_atomic_drops_and_keeps_order() {
        ...
        assert_eq!(dropped, 1);
        assert_eq!(std::fs::read(&p).expect("读"), b"a\nc\n");
