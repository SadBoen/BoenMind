...
fn layer_of(crate_name: &str) -> Option<u32> {
    Some(match crate_name {
        ...
        | "plugin-host-tools" | "plugin-code-runtime" | "plugin-web-tools" | "plugin-schedule" | "plugin-goal"
        | "plugin-approval" => 2,
        "bm-assembly" => 1,
...
fn is_feature_plugin(crate_name: &str) -> bool {
    matches!(crate_name, "plugin-auth" | "plugin-compactor" | "plugin-code-runtime" | "plugin-web-tools" | "plugin-schedule" | "plugin-goal" | "plugin-approval")
}
