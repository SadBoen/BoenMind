...
function createFishLogo(): HTMLElement {
...
function removeFishLogo(host: HTMLElement): void {
...
function fileIcon(name: string, directory = false): HTMLElement {
  ...
  else if (['conf', 'config', 'ini', 'env', 'toml'].includes(extension ?? '') || name.startsWith('.')) { icon.className += ' dfb-icon-config'; icon.textContent = '⚙' }
...
function createPanel(ctx: ClientContext): { root: HTMLElement; openFile(path: string): Promise<void>; dispose(): void } {
  const style = document.createElement('style'); style.textContent = css + reviewCss + themeCss; document.head.append(style)
   ...
   */
  const rowItem = (header: HTMLElement, rootElement: HTMLElement): HTMLElement => {
    let item: HTMLElement = header
    while (item.parentElement !== rootElement && item.parentElement !== null) item = item.parentElement
  ...
  const mountTab = (): void => {
    ...
    while (candidate !== null && candidate.querySelector('[data-conversation-scroll]') === null) candidate = candidate.parentElement
