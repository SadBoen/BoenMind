...
fmt:              --check 通过
live:             t144 实网流式通过 + 真实浏览器端到端手测通过
                  (网页打字机渲染逐段出现 ▌ 光标,72 增量事件全落账;
                  实网调用合计 4 次,均 max_tokens 受限)
```
...
4. **性能是否触门?** 否。全量复跑七项(P-01..P-05/07/08)门内,记录⑦
   已入 perf-baseline(S2/S3 面默认关或旁路,热路径零新增)。
...
- 发现并修复:delta 事件初版未携带 session_id,被 events.poll 的会话过滤
  (X-02 隔离纪律)正确地挡在会话流之外——修复为发射时从收据回填会话归属。
- 服务器启动日志增印「模型流式 = true/false」(可观测性小改)。
- 教训重申:测试全绿 ≠ 用户可见;真实浏览器手测轮不可省(AGENTS.md 踩坑)。
