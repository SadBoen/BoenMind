...
> 合同库(第 1 层)之下;与两者冲突时,以上两层为准。标注 **[裁决]** 的条目
> 为本规格新增的技术决策;标注 **[开放]** 的条目待用户裁决后转正。
> 状态:**草案**——经用户评审后冻结为 v1.0,方可开工写代码。
...
### 4.3 模型连接器:mock 为主,真实为辅 **[开放 D1]**

- 默认交付:`MockConnector`,脚本化响应/超时/失败序列——GT-01 场景 A/B、
  降级链、INV-4 全部可确定性回放;P-01..08 性能数值按 perf-baseline 约定以
  mock 回填。
- **D1(待裁决)**:真实 GLM HTTP 适配器是否列入 M1 交付?
  建议:列入,但 `feature = "glm"` 门控、默认关闭、不参与 P-01..08 定标;
  M1 验收不依赖外网。备选:推迟到 M2,唯一代价是 M1 演示走 mock。
...
| # | 问题 | 建议 |
|---|---|---|
| D1 | 真实 GLM 适配器是否列入 M1 | 列入但 feature 门控,默认关,验收不依赖外网 |
| D2 | crate 前缀 `bm-` 是否可用 | 短前缀利于内部引用;对外发布名 M3 再定 |
| D3 | M1 的 resume 语义边界 | 仅同进程内 resume;跨进程恢复明确归 M2,写进规格避免验收争议 |
