...
> 状态:**草案**——用户评审冻结后开工;自主推进期间按 §9 默认路径先行。
...
## 9. 自主推进默认路径(开放裁决期间的先行依据)

- D-M3-3(TLS 终止位置):默认回环绑定 + 反代终止;PENDING 已记。
- D-M3-1(watch 形态):默认 SSE;若用户倾向长轮询,T4 前提出即可,改动局部。
- D-M3-2(Tauri 范围):默认最小三视图;UX 演进归 M8。
- 合同增发(T0)仍为 Minor 纯追加,可逆。
