window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		const TreeRow = (0, react.memo)(TreeRowBase, (prev, next) => prev.expanded.has(prev.entry.path) === next.expanded.has(next.entry.path) && prev.entry === next.entry && prev.depth === next.depth && prev.selected === next.selected && prev.dirs === next.dirs && prev.root === next.root && prev.stores === next.stores && prev.onContextMenu === next.onContextMenu);
		//#endregion
