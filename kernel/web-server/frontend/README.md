...
- `index.html` —— 壳层入口，**已含 `window.__DSH_BOOT__` 注入**（38 个插件 entry，
  rev `dcccb8324e44`；`<` 已按 dsh 规则转义 `\u003c`）。
...
快照取自 dsh Node 后端（`dsh --profile web`）真实下发内容：

1. 启动 Node 后端：`cd dsh-home/profiles/web && node ../../profiles/node_modules/@deepseek-ai/dsh/lib/bin.js web --port 3090`
2. `curl http://127.0.0.1:3090/` 取注入后的 index.html，解析 `__DSH_BOOT__` 的 38 个 entry
3. 逐 entry `GET /plugins/<id>/client.js?rev=...` 落盘
4. 壳层静态资产（assets/favicon/manifest）从已发布 npm 包
   `@deepseek-ai/dsh-web-frontend` 的 `dist/` 拷贝

dsh 版本升级后按上述步骤重抓刷新（快照 rev 与 index.html/boot.json 需同步更新）。
