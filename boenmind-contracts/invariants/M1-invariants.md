...
| INV-13 | 回喂忠实性(ADR-0029):内核写入 tool 角色消息与错误信封的内容只承载事实——工具原文、真实错误(经凭据脱敏)、未执行原因;内核不得合成 assistant 终稿、不得硬编码指导性话术,此类文本只能经插件/配置通道以可区分方式注入 | 构造点审计 + 脱敏与边界测试 | 5.4/ADR-0029 |
